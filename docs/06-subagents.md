...
## 🚀 応用：Ruflo Swarm — 100+ エージェントの自己組織化

Subagent を「1 つずつ呼ぶ」のではなく、**群（Swarm）として自己組織化** させる方式が、[`ruvnet/ruflo`](https://github.com/ruvnet/ruflo) (50k★、旧 Claude Flow) で実装されています:
...
# フル導入 (MCP server / 98 agents / 60+ commands / Hooks)
npx ruvflo init
```

Ruflo の Hooks system が **タスクを自動ルーティング → 成功パターンを学習 → バックグラウンドでエージェント調整** を行うため、ユーザは Claude Code を普段通り使うだけ。

詳細・応用例: [`16-case-studies.md`](./16-case-studies.md) (Case 2)
